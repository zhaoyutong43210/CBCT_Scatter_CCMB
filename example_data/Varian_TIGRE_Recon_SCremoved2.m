function Varian_TIGRE_Recon_SCremoved2
...
if isfile('Simu_dataset2.mat')
    load('Simu_dataset2.mat')
% load scattering intensity 
else
    [scatt_int,simuproj,clean] = ScatterDataLoader(data_scatter_folder);
    save('Simu_dataset2.mat','scatt_int','simuproj','clean','-v7.3')
end

% use load
if isfile('Varian_proj_info.mat')
    load('Varian_proj_info.mat');
else
    if ~motion_lag
    [proj,Iproj,blk, geo, angles] = VarianDataLoader2(datafolder);
    else
    [proj,Iproj,blk, geo, angles] = VarianDataLoader2(datafolder, false); %remove motion lag correction.
    end 
    
    save('Varian_proj_info.mat','proj','Iproj','blk','geo','angles','-v7.3');
end
...
proj_clean = log(repmat(blk, [1 1 size(clean,3)])./clean);
...
function [scatter,att,clean] = ScatterDataLoader(datafolder)
